...
3. **일 1회(프로덕션, US Pacific 오전 8시)**: `daily-menu.yml`은 매일 **별도의** `cron` 두 줄(`5 15 * * *`, `5 16 * * *`)로 **UTC 15:05·16:05**에 실행됩니다(일광절약제·표준시 각각 8시 전후). `ENFORCE_LOCAL_NOTIFY_WINDOW`가 켜진 **스케줄 실행**에서는 `GHA_SCHEDULE_CRON`으로 어느 UTC 슬롯인지 구분합니다: PDT에는 15 UTC 슬롯만 전송(8시대·지연으로 9시대도 허용), 16 UTC 슬롯은 9시 전후라 스킵; PST에는 반대로 16 UTC 슬롯만 8시에 전송합니다. GitHub 스케줄이 밀려도 15 UTC 작업이 9시 전후까지 늦게 돌면 전송됩니다. **Run workflow** 수동 실행은 시간 제한 없이 항상 전송합니다. 알림 시각을 바꾸면 `MENU_NOTIFY_HOUR`와 `cron`·`main.py` 슬롯 규칙을 함께 맞추세요.
