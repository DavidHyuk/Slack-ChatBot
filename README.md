...
3. **프로덕션(`daily-menu.yml`)**: 매일 **UTC 15:05** 한 번(`5 15 * * *`)만 실행합니다. America/Los_Angeles 기준 **일광절약제(PDT) 중에는 대략 오전 8시 전후**, 표준시(PST) 겨울에는 **같은 UTC라서 대략 오전 7시 전후**에 가깝습니다(PST/PDT 분기 없음). 실행되면 곧바로 Slack으로 보냅니다. **Run workflow** 수동 실행도 동일합니다.
4. **약 오전 10시(PDT) 스모크 테스트**: `daily-menu-pacific-10am-test.yml`은 **UTC 17:05** 스케줄 + **Run workflow**로 같은 Secrets로 시험합니다. 크론 시각 확인은 `python3 scripts/pacific_cron_preview.py --date 2026-04-22` 등.
